  ...
  $.fn.ImageEditor = function(options) {
    ...
    function ImageEditor($el, options) {
      ...
      var defaults = {
        ...
        removeIconSize: 0,
        addImageAnimation: false,
        removeImageAnimation: false,
    ...
    ImageEditor.prototype = {
      ...
      addImage: function(url, select, scale) {
        ...
        var index = that.images.length-1;
      ...
      setImage: function(url, index, select, scale, position) {
        ...
        $img.on('load', function() {
          ...
          if(position){
            image.transform.translate.x = position.x;
            image.transform.translate.y = position.y;
          }
          that._updateImageTransform(image);
        });
      ...
      setImageByObject: function(imageBase64, index, select, scale, position) {
        if(index > this.images.length - 1)
          // throw new Error('index out of range');
          return;
        ...
        var image = this.images[index],
          ...
          image = {
            id: new Date() * 1,
            url: 'data:image/png;base64, '+imageBase64,//url.url,
            closeButtonRequire: url.closeButtonRequire,
            clickToSelect: url.clickToSelect,
            onClick: url.onClick,
            img: $(img)[0],
          ...
          image.$imgWrapper = $('<span>').append($(img));
          that.images[index] = image;
          that._placeImage(image);
